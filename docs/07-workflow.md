...
| Изменение frontend-кодстайла                                | `eslint.config.js` / `.prettierrc.json` / `stylelint.config.js` / `docs/frontend-styleguide.md` и связанные frontend docs |
...
## Правило UI/UX контракта

- `docs/11-ui-ux-contract.json` является каноническим JSON-описанием текущих UI/UX элементов, пригодным для передачи дизайнеру.
- При добавлении, удалении, переименовании или поведенческом изменении любого UI элемента агент обязан обновить `docs/11-ui-ux-contract.json` в той же задаче.
- Нельзя считать UI-задачу завершенной, если изменился интерфейс, а JSON-контракт не синхронизирован.
